...
class BasicBlock_dw_1(nn.Module):
    def __init__(self,
                 in_channels, out_channels,
                 ksize=3, stride=1, pad=1, dilation=1):
        super(BasicBlock_dw_1, self).__init__()
...
class BasicBlockSig_dw(nn.Module):
    def __init__(self,
                 in_channels, out_channels,
                 ksize=3, stride=1, pad=1):
        super(BasicBlockSig_dw, self).__init__()

        self.body = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, ksize, stride, pad, groups = in_channels),
            nn.Conv2d(in_channels, out_channels, 1, 1, 0),
            nn.Sigmoid()
        )
